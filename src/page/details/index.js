  ...
  render() {
        ...
        render(text, record, index) {
          ...
          return <div>
            {content.map((i, index) => {
              const item = i ? 
                i.indexOf("@") !== -1 ?
                `${ util.filterExactAcc(i.split("@")[0])}@active` 
                : i
              : "";
    ...
    const columnsAction = [
      ...
      },
      {
        ...
        key: "data",
        render: (data, record) => {
          // const data = oridata.replace('eosio.token', 'dmc.token')
          console.log(data)
          if (data.length === 4) {
            if (data[0] === this.state.account_name) {
              return (
                <p>
                  <span className="dataColor">{util.filterExactAcc(data[0])}</span>{" "}
                  <Icon type="arrow-right" /> <span>{util.filterExactAcc(data[1])}</span>
                  ，&nbsp;&nbsp; {intl.get("amount")}：
                  <span className="dataColor">{util.filterExactAcc(data[2])}</span>，
                  &nbsp;&nbsp;&nbsp; {intl.get("remarks")}：
            ...
            if (data[1] === this.state.account_name) {
              return (
                <p>
                  <span>{util.filterExactAcc(data[0])}</span> <Icon type="arrow-right" />{" "}
                  <span className="dataColor">{util.filterExactAcc(data[1])}</span>，&nbsp;&nbsp;
                  {intl.get("amount")}：
                  <span className="dataColor">{util.filterExactAcc(data[2])}</span>，
                  &nbsp;&nbsp;&nbsp; {intl.get("remarks")}：
                  <span className="memoColor">{data[3]}</span>
            ...
            if (typeof data === "string") {
              return <p>{util.filterSysAcc(JSON.stringify(data.substr(0, 12) + "..."))}</p>;
            } else {
              return <p> {util.filterSysAcc(JSON.stringify(data, null, 4))}</p>;
            }
          }
        }
      },
    ...
    return (
      <div className="accountsdetails">
        ...
        <div className="origindata">
          ...
          <ReactJson 
            src={JSON.parse(
              util.filterSysAcc(`${JSON.stringify(this.state.data)}`)
            )} 
            collapsed={true} 
          />
        </div>
        ...
        >
          <div>
            <pre>
              <code>{util.filterSysAcc(JSON.stringify(this.state.obj, null, 4))}</code>
            </pre>
          </div>
        </Modal>
      </div>
